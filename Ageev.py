...
def calcHist(array):       # Создаем функцию Calchist для подсчета частоты вхождения элементов в интервалы
    array_2 = [0] * 10
    for i in array:
        if i <= 99:
            array_2[0] += 1
...
def make_list():         # Создаем функцию make_list для создания массива из 1000000 элементов
    array_4 = []
...
time_array = []
for i in range(100):     # Создаем функцию time_array для вычисления длительности работы calcHist и его прогона количестве 100 раз
    array_1 = make_list()
    t0 = time.time()
    calcHist(array_1)
